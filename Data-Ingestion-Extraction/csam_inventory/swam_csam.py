...
SYSTEM_APPENDIX_LINK_ID = (
    ...
    "ctl00_ctl72_hlViewArt"
)

ID_ORG_ACRONYM_FILE_NAME = "id-org-acronym.csv"
...
class CsamScraper(LoggingBase):
    ...
    def collect_software_inventories(self, system_list: List[int]) -> None:
        """Download software inventories for multiple systems
        ...
        for i, system in enumerate(system_list):
            logging.info(
                "Downloading CSAM software inventory for system %s (%d/%d).",
                system, i+1, total
            )

            self._collect_software_inventory(system)

    def _collect_software_inventory(self, system_id: int) -> None:
        """Download the software inventory for a single system

        Parameters
        ----------
        system_id: int
            ID of the system whose software inventory should be downloaded
        """
        logging.debug("Retrieving CSAM software inventory for system %d.",
                      system_id)
        ...
        hw_file.rename(download_path / f"sw-inventory-{system_id}{extension}")

        logging.debug("Software inventory for system %d downloaded.", system_id)
