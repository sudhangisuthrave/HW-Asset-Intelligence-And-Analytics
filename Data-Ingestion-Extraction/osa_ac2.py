...
from csam_osa_report import osa, osa_extract, osa_utils
...
def download_csam_inventories(config: Dict) -> None:
    ...
    """
    scraper = osa.CsamScraper(config)
    scraper.login()
...
def process_inventories(config: Dict) -> Dict[int, List[str]]:
    ...
    for item in output_path.iterdir():
        if (not item.is_file()
                or item.name.lower() == osa.ID_ORG_ACRONYM_FILE_NAME):
            continue
