...
run_script('generate_hosts_01192024.py', 'CSAM-org-acronym.xlsx', 'CombinedFile-CSAMTemplate.xlsx', '.')
...
df2 = pd.read_csv('clear_tuple_host_new_template.csv', skiprows=1).dropna(how='all')  # Drop rows where all values are NaN
df3 = pd.read_csv('host_data.csv', skiprows=1).dropna(how='all')  # Drop rows where all values are NaN
...
# Read the master_output.csv and replace specified strings
master_df = pd.read_csv('master_output.csv', encoding='utf-8')  # specify the encoding
master_df.replace({' â€“ ': ' - ', ' â ': ' - '}, inplace=True)

# Save the updated CSV file
master_df.to_csv('master_output_updated.csv', index=False)